...
events.on(
	'product:addToBasket',
	async ({ previewCard, card }: { previewCard: Card; card: ICard }) => {
		try {
...
events.on('order:begin', () => {
	orderModel.items = basketModel.products.map((item) => {
...
events.on(
	'payment:change',
	(data: { field: keyof IDeliveryForm; value: PaymentMethods }) => {
...
events.on(
	'deliveryFormErrors:change',
	(data: { errors: Partial<IDeliveryForm>; valid: boolean }) => {
...
events.on(
	...
	(data: { errors: Partial<IContactsForm>; valid: boolean }) => {
